...
def main():
    ...
    for jsonl_file in jsonl_files:
        ...
        for obj in data:
            ...
            for gt_behav, gt_start, gt_end in gt_maneuvers:
                if gt_behav not in action_stats:
                    action_stats[gt_behav] = {'correct': 0, 'total': 0, 'ious': []}
                action_stats[gt_behav]['total'] += 1
                
                # 检查是否有匹配
                same_pred = [(pred_start, pred_end) for pred_behav, pred_start, pred_end in pred_maneuvers if pred_behav == gt_behav]
                if not same_pred:
                    continue
                # 有同名行为，计算最大IOU
                max_iou = 0
                for pred_start, pred_end in same_pred:
                    current_iou = iou(gt_start, gt_end, pred_start, pred_end)
                    if current_iou > max_iou:
                        max_iou = current_iou
                if max_iou >= 0:
                    action_stats[gt_behav]['correct'] += 1
                    action_stats[gt_behav]['ious'].append(max_iou)
    ...
    file_names = list(all_file_stats.keys())
    ...
    ax.set_title('Recall by Behavior (Grouped Comparison)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig('/root/workspace/LLaMA-Factory/VQA/plot/recall_bar_chart.png')
    print("Bar chart saved as recall_bar_chart.png")
