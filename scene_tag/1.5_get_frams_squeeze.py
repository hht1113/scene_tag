...
class VideoDownsampler:
    """视频降采样工具，从10fps降为1fps，并调整分辨率为448x448"""
    
    def __init__(self, target_fps=1, original_fps=10, target_resolution=(448, 448)):
        """
        初始化降采样器
        ...
        Args:
            ...
            target_resolution: 目标分辨率 (默认448x448)
        """
    ...
    def downsample_video_opencv_robust(self, input_path, output_path=None):
        ...
        # 检查是否已经是目标帧率和分辨率
        if (abs(video_info['fps'] - self.target_fps) < 0.1 and 
            video_info['width'] == target_w and 
            video_info['height'] == target_h):
            logger.info(f"视频已经是{self.target_fps}fps和{target_w}x{target_h}分辨率，跳过处理: {input_path}")
            return True
        ...
        try:
            ...
            logger.info(f"处理完成: 读取{frame_idx}帧, 保存{saved_frame_idx}帧, 分辨率调整为{target_w}x{target_h}")
            ...
            if os.path.exists(output_path):
                ...
                if processed_size > 0:
                    ...
                    if processed_info:
                        ...
                        # 验证分辨率是否正确
                        if processed_info['width'] != target_w or processed_info['height'] != target_h:
                            logger.warning(f"分辨率不匹配: 期望{target_w}x{target_h}, 实际{processed_info['width']}x{processed_info['height']}")
    ...
    def downsample_video_ffmpeg_simple(self, input_path, output_path=None):
        ...
        try:
            ...
            cmd_with_audio = [
                ...
                '-vf', f'scale={target_w}:{target_h}',  # 调整分辨率
                '-c:v', 'libx264',           # 视频编码
                '-preset', 'fast',           # 编码速度
                '-crf', '23',               # 质量
                '-c:a', 'copy',             # 复制音频
        ...
        for video_path in tqdm(video_files, desc="处理视频", unit="个"):
            try:
                ...
                if (abs(video_info['fps'] - self.target_fps) < 0.1 and 
                    video_info['width'] == target_w and 
                    video_info['height'] == target_h):
                    logger.debug(f"视频已经是{self.target_fps}fps和{target_w}x{target_h}分辨率，跳过: {video_path}")
                    stats['skipped'] += 1
                    continue
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='视频降采样工具 - 从10fps降到1fps，并调整分辨率为448x448')
    ...
    print("=" * 60)
    print("视频降采样工具 - 从10fps降到1fps，调整分辨率为448x448")
    print("=" * 60)
    ...
    for video_path in tqdm(video_files, desc="处理视频", unit="个"):
        try:
            ...
            target_w, target_h = downsampler.target_resolution
            if (abs(video_info['fps'] - args.target_fps) < 0.1 and 
                video_info['width'] == target_w and 
                video_info['height'] == target_h):
                logger.debug(f"视频已经是{args.target_fps}fps和{target_w}x{target_h}分辨率，跳过: {video_path}")
                skipped_count += 1
                continue
    ...
    log_data = {
        ...
        'target_fps': args.target_fps,
        'original_fps': args.original_fps,
        'target_resolution': f"{args.width}x{args.height}",
