"""
构造多标签微调数据集 v3：使用 system_prompt_v3.txt，移除 NavForIntersection 和 LaneCruising。

主要变化 (相比 v2):
  - 使用 system_prompt_v3.txt（新增消歧规则，解决 UTurn/LeftTurn/VRUInLaneCrossing 混淆）
  - 去掉 LaneChange_NavForIntersection 和 LaneCruising_Straight 两个标签
  - 测试集使用已人工修正过的 v3 版本作为基础
"""
...
SAMPLING_MULTIPLIERS = {
    "TrafficLight_StraightStopOrGo":                0.52,
    "TrafficLight_LeftTurnStopOrGo":                0.50,
    "LaneChange_AvoidSlowVRU":                      3.5,
    "LaneChange_AvoidStaticVehicle":                0.72,
    "DynamicInteraction_VRUInLaneCrossing":          1.35,
    "DynamicInteraction_VehicleInLaneCrossing":      0.72,
    "DynamicInteraction_StandardVehicleCutIn":       0.97,
    "StartStop_StartFromMainRoad":                  5.7,
    "StartStop_ParkRoadside":                       3.1,
    "Intersection_StandardUTurn":                   0.51,
}

DEFAULT_MULTIPLIER = 1.0
ALLOWED_LABELS = set(SAMPLING_MULTIPLIERS.keys())

SYSTEM_PROMPT_FILE = Path(__file__).parent / "system_prompt_v3.txt"
...
def main():
    ...
    print("[0/8] 加载 system_prompt_v3.txt...")
    ...
    print("\n[3/8] 替换所有样本的 system prompt 为 v3...")
    ...
    # ── Step 5.5: 应用人工修正到测试集 ──
    print(f"\n[5.5/8] 应用人工修正的 GT 标注...")
    v3_corrected_path = data_dir / "qwen3_sft_test_segment_multilabel_v3.json"
    if v3_corrected_path.exists():
        with open(v3_corrected_path) as f:
            v3_corrected = json.load(f)
        v3_corrections = {}
        for item in v3_corrected:
            vid = item["videos"][0]
            v3_corrections[vid] = item

        corrected_count = 0
        for i, item in enumerate(new_test):
            vid = item["videos"][0]
            if vid in v3_corrections:
                ref = v3_corrections[vid]
                if (item.get("all_labels") != ref.get("all_labels") or
                    item.get("output") != ref.get("output")):
                    new_test[i]["output"] = ref["output"]
                    new_test[i]["all_labels"] = ref["all_labels"]
                    new_test[i]["label_en"] = ref["label_en"]
                    new_test[i]["time_range_in_slice"] = ref["time_range_in_slice"]
                    new_test[i]["num_labels"] = ref["num_labels"]
                    corrected_count += 1

        print(f"      已应用 {corrected_count} 条人工修正")
    else:
        print(f"      未找到 v3 修正文件, 跳过")

    # 更新测试集 system prompt
    update_system_prompts(new_test, new_system_prompt)
    ...
    # 更新训练集 system prompt
    update_system_prompts(sampled_train, new_system_prompt)
    ...
    test_out = data_dir / "qwen3_sft_test_segment_multilabel_v3.json"
    train_out = data_dir / "qwen3_sft_train_segment_multilabel_v3.json"
