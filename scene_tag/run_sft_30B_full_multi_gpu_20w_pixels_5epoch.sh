#!/bin/bash
###############################################################################
# Qwen3-VL-30B-A3B-Instruct 全参 SFT 多卡分布式训练脚本
#
# 数据集:   qwen3_sft_train_segment_upsample.json
# 微调方式: 全参微调 + DeepSpeed ZeRO-3 Offload (20万 pixels/帧)
# 续训:     从2epoch checkpoint-226断点续训到5epoch
# 框架:     LLaMA-Factory + torchrun 分布式训练
#
# 用法:
#   8卡训练:  bash run_sft_30B_full_multi_gpu_20w_pixels_5epoch.sh
#   指定GPU:  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_sft_30B_full_multi_gpu_20w_pixels_5epoch.sh
#   多机训练: NNODES=2 NODE_RANK=0 MASTER_ADDR=192.168.0.1 MASTER_PORT=29500 bash run_sft_30B_full_multi_gpu_20w_pixels_5epoch.sh
###############################################################################
...
CONFIG_FILE="${SCRIPT_DIR}/train_qwen3vl_30B_full_multi_gpu_20w_pixels_5epoch.yaml"
...
# ========================= 环境检查 =========================
echo "============================================================"
echo "  Qwen3-VL-30B-A3B 全参 SFT 多卡训练 (DeepSpeed ZeRO-3 Offload, 20w pixels, 续训到5epoch)"
echo "============================================================"
echo ""
...
# 检查 DeepSpeed 配置文件
DS_CONFIG="${PROJECT_DIR}/examples/deepspeed/ds_z3_offload_config.json"
if [ ! -f "$DS_CONFIG" ]; then
