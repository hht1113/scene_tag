#!/bin/bash
###############################################################################
# Qwen3-VL-30B-A3B 全参 SFT 多卡分布式训练脚本 (ZeRO-3 快速版，无offload)
#
# 数据集:   qwen3_sft_train_segment_upsample.json
# 微调方式: 全参微调 + DeepSpeed ZeRO-3 (overlap_comm=true, 无CPU offload)
# 框架:     LLaMA-Factory + torchrun 分布式训练
#
# 用法:
#   8卡训练:  bash run_sft_30B_full_multi_gpu_z3_fast.sh
#   指定GPU:  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_sft_30B_full_multi_gpu_z3_fast.sh
#   多机训练: NNODES=2 NODE_RANK=0 MASTER_ADDR=192.168.0.1 MASTER_PORT=29500 bash run_sft_30B_full_multi_gpu_z3_fast.sh
###############################################################################
...
CONFIG_FILE="${SCRIPT_DIR}/train_qwen3vl_30B_full_multi_gpu_z3_fast.yaml"

cd "$PROJECT_DIR"

# ========================= 环境检查 =========================
echo "============================================================"
echo "  Qwen3-VL-30B-A3B 全参 SFT 多卡训练"
echo "  DeepSpeed ZeRO-3 快速版 (overlap_comm=true, 无CPU offload)"
echo "============================================================"
echo ""
