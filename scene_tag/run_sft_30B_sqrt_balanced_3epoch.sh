#!/bin/bash
###############################################################################
# Qwen3-VL-30B-A3B 续训脚本: sqrt 温度平衡采样 + 3 epoch
#
# 基座: add_tags_dynamic 2epoch 检查点
# 数据: sqrt 温度重采样 (10121 样本, 12 类, Max/Min=4.4x)
# DS:   ZeRO-3 (overlap_comm=true, 无 CPU offload) — 更快
#
# 用法:
#   bash run_sft_30B_sqrt_balanced_3epoch.sh
###############################################################################
...
CONFIG_FILE="${SCRIPT_DIR}/train_qwen3vl_30B_full_multi_gpu_sqrt_balanced_3epoch.yaml"
...
# ========================= 环境检查 =========================
echo "============================================================"
echo "  Qwen3-VL-30B-A3B 续训 (sqrt 平衡采样 + 3epoch)"
...
DATASET_FILE="${PROJECT_DIR}/data/qwen3_sft_train_segment_add_tags_sqrt_balanced.json"
if [ ! -f "$DATASET_FILE" ]; then
    echo "[ERROR] 数据集文件不存在: $DATASET_FILE"
    echo "[INFO] 请先运行: python3 scene_tag/5_resample_dataset_sqrt_balanced.py"
    exit 1
fi
