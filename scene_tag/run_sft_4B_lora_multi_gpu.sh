#!/bin/bash
###############################################################################
# Qwen3-VL-4B-Instruct LoRA SFT 多卡分布式训练脚本
#
# 数据集:   qwen3_sft_train_segment_upsample_with_review.json
# 微调方式: LoRA 微调 + 多卡 DDP
# 框架:     LLaMA-Factory + torchrun 分布式训练
#
# 用法:
#   8卡训练:  bash run_sft_4B_lora_multi_gpu.sh
#   指定GPU:  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_sft_4B_lora_multi_gpu.sh
#   多机训练: NNODES=2 NODE_RANK=0 MASTER_ADDR=192.168.0.1 MASTER_PORT=29500 bash run_sft_4B_lora_multi_gpu.sh
###############################################################################
...
CONFIG_FILE="${SCRIPT_DIR}/train_qwen3vl_4B_lora_sft_8gpu.yaml"
...
# ========================= 环境检查 =========================
echo "============================================================"
echo "  Qwen3-VL-4B LoRA SFT 多卡训练"
echo "============================================================"
echo ""
